	...
	!	WRITE(*,*) myid, ' COLLISION ', x_now(3)
		...
		IF (CM_TYPE .EQ. 'QM') THEN
			...
			ELSE IF (TRIM(PROJ) .EQ. 'O') THEN
				IF (TRIM(Targ) .EQ. 'H') THEN
					CALL lin_rand_angle( E_now, 'OH    ', ScattAng )
				ELSE IF (TRIM(Targ) .EQ. 'He') THEN
					CALL lin_rand_angle( E_now, 'OHe   ', ScattAng )
				ELSE IF (TRIM(Targ) .EQ. 'O') THEN
					CALL lin_rand_angle( E_now, 'OO    ', ScattAng )
				ELSE IF (TRIM(Targ) .EQ. 'Ar') THEN
					CALL lin_rand_angle( E_now, 'OAr   ', ScattAng )
				ELSE IF (TRIM(Targ) .EQ. 'H2') THEN
					CALL lin_rand_angle( E_now, 'OH2   ', ScattAng )
				ELSE IF (TRIM(Targ) .EQ. 'N2') THEN
					CALL lin_rand_angle( E_now, 'ON2   ', ScattAng )
				ELSE IF (TRIM(Targ) .EQ. 'CO') THEN
					CALL lin_rand_angle( E_now, 'OCO   ', ScattAng )
				ELSE IF (TRIM(Targ) .EQ. 'CO2') THEN
					CALL lin_rand_angle( E_now, 'OCO2  ', ScattAng )
				END IF
			ELSE IF (TRIM(PROJ) .EQ. 'H2O') THEN
				IF (TRIM(Targ) .EQ. 'H') THEN
					CALL lin_rand_angle( E_now, 'H2OH  ', ScattAng )
				ELSE IF (TRIM(Targ) .EQ. 'He') THEN
					CALL lin_rand_angle( E_now, 'H2OHe ', ScattAng )
				ELSE IF (TRIM(Targ) .EQ. 'O') THEN
					CALL lin_rand_angle( E_now, 'H2OO  ', ScattAng )
				ELSE IF (TRIM(Targ) .EQ. 'Ar') THEN
					CALL lin_rand_angle( E_now, 'H2OAr ', ScattAng )
				ELSE IF (TRIM(Targ) .EQ. 'H2') THEN
					CALL lin_rand_angle( E_now, 'H2OH2 ', ScattAng )
				ELSE IF (TRIM(Targ) .EQ. 'N2') THEN
					CALL lin_rand_angle( E_now, 'H2ON2 ', ScattAng )
				ELSE IF (TRIM(Targ) .EQ. 'CO') THEN
					CALL lin_rand_angle( E_now, 'H2OCO ', ScattAng )
				ELSE IF (TRIM(Targ) .EQ. 'CO2') THEN
					CALL lin_rand_angle( E_now, 'H2OCO2', ScattAng )
				END IF
			END IF
		ELSE IF (CM_TYPE .EQ. 'HS') THEN
