...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	...
	IF ( (TRIM(Coll_Type) .EQ. 'HCO2') .OR. (TRIM(Coll_Type) .EQ. 'HeCO2') ) THEN
		...
		Ang_now(:) = F_ANGLE(:)
	END IF
	IF ( (TRIM(Coll_Type) .EQ. 'OH') .OR. (TRIM(Coll_Type) .EQ. 'H2OH') ) THEN
		PD_now(:)  = F_PD_X_H(E_index,:)
		Ang_now(:) = F_ANGLE(:)
	END IF
	IF ( (TRIM(Coll_Type) .EQ. 'OHe') .OR. (TRIM(Coll_Type) .EQ. 'H2OHe') ) THEN
		PD_now(:)  = F_PD_X_He(E_index,:)
		Ang_now(:) = F_ANGLE(:)
	END IF
	IF ( (TRIM(Coll_Type) .EQ. 'OO') .OR. (TRIM(Coll_Type) .EQ. 'H2OO') ) THEN
		PD_now(:)  = F_PD_X_O(E_index,:)
		Ang_now(:) = F_ANGLE(:)
	END IF
	IF ( (TRIM(Coll_Type) .EQ. 'OAr') .OR. (TRIM(Coll_Type) .EQ. 'H2OAr') ) THEN
		PD_now(:)  = F_PD_X_Ar(E_index,:)
		Ang_now(:) = F_ANGLE(:)
	END IF
	IF ( (TRIM(Coll_Type) .EQ. 'OH2') .OR. (TRIM(Coll_Type) .EQ. 'H2OH2') ) THEN
		PD_now(:)  = F_PD_X_H2(E_index,:)
		Ang_now(:) = F_ANGLE(:)
	END IF
	IF ( (TRIM(Coll_Type) .EQ. 'ON2') .OR. (TRIM(Coll_Type) .EQ. 'H2ON2') ) THEN
		PD_now(:)  = F_PD_X_N2(E_index,:)
		Ang_now(:) = F_ANGLE(:)
	END IF
	IF ( (TRIM(Coll_Type) .EQ. 'OCO') .OR. (TRIM(Coll_Type) .EQ. 'H2OCO') ) THEN
		PD_now(:)  = F_PD_X_CO(E_index,:)
		Ang_now(:) = F_ANGLE(:)
	END IF
	IF ( (TRIM(Coll_Type) .EQ. 'OCO2') .OR. (TRIM(Coll_Type) .EQ. 'H2OCO2') ) THEN
