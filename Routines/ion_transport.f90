SUBROUTINE ion_transport_test
	...
	REAL(KIND=8)			:: tt, amu, E, CX, R0(3), V0(3), rout(3), vout(3), path, den
	INTEGER						:: Z, i, N
	...
	DO i=1,N
		CALL ion_transport( amu, Z, E, den, CX, R0, V0, rout, vout, tt, path )
...
!
!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
SUBROUTINE ion_transport( amu, Z, E, den, CX, R0, V0, rout, vout, tt, ion_path )
	USE ENA
	USE physics_constants, ONLY : PI, MTOAU
	USE mpi_info 
...
!	WRITE(*,*) 'rc: rp: rt: ', RC, rp, rt
	rout = RC + rp + rt
!	WRITE(*,*) 'IN IT: rx: ry: rz: ', rout(1)*MTOAU, rout(2)*MTOAU, rout(3)*MTOAU
!	WRITE(*,'(A,3ES10.2)') 'RC: ', RC(1), RC(2), RC(3)
!	WRITE(*,'(A,3ES10.2)') 'RP: ', rp(1), rp(2), rp(3)
!	WRITE(*,'(A,3ES10.2)') 'RT: ', rt(1), rt(2), rt(3)
