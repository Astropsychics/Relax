...
SUBROUTINE SW_Hp_ENA_prod(z,f)
  ...
  REAL(KIND=8)  :: now, tot, dx, x, r, top, bot, VSW, cx_H2, cx_N2
	REAL(KIND=8)	:: Elab, E_CM_H, E_CM_He, E_CM_O, E_CM_Ar
	REAL(KIND=8)	:: E_CM_H2, E_CM_N2, E_CM_CO, E_CM_CO2, pp, tt, mm
  CHARACTER     :: ion*10
  INTEGER       :: N, i, j
...
!  VSW   = SQRT(E*EVTOJ/(2.0D0*Mass_p*TOAMU*AMUTOKG))
  ...
  !! Get CX CS for each collision at current energy
  CALL cx_cross_sections( ion, 'H  ', 1, E_CM_H, cx_H )
  CALL cx_cross_sections( ion, 'O  ', 1, E_CM_O, cx_O )
  CALL cx_cross_sections( ion, 'He ', 1, E_CM_He, cx_He )
  CALL cx_cross_sections( ion, 'CO2', 1, E_CM_CO2, cx_CO2 )
  CALL cx_cross_sections( ion, 'H2 ', 1, E_CM_H2, cx_H2 )
  CALL cx_cross_sections( ion, 'CO2', 1, E_CM_CO2, cx_N2 )
  CALL cx_cross_sections( ion, 'CO ', 1, E_CM_CO, cx_CO )
  CALL cx_cross_sections( ion, 'Ar ', 1, E_CM_Ar, cx_Ar )
...
SUBROUTINE SW_Hepp_ENA_prod(z,f)
  ...
  REAL(KIND=8)  :: now, tot, dx, x, r, top, bot, VSW
	REAL(KIND=8)	:: pp, tt, mm, Elab, E_CM_He, E_CM_CO2
  CHARACTER     :: ion*10
  INTEGER       :: N, i, j
...
!	E     = E/EVTOJ !! convert from J to eV
  ...
  !! Get CX CS for each collision at current energy
  CALL cx_cross_sections( ion, 'He ', 2, E_CM_He, cx_He )
	CALL cx_cross_sections( ion, 'CO2', 2, E_CM_CO2, cx_CO2 ) 
