...
SUBROUTINE planet_onestep_3d
	...
	REAL(KIND=8)	:: R1_L, R1_U, R2_L, R2_U, R3_L, R3_U, Deepest
...
!			CALL test_SH_Energy_Prob
	...
	!! set deepest counters to high altitude to start [m]
	Deepest = 1000.0D3
...
!			IF ( (E_now .GE. E_Therm) .AND. (x_now(3) .LE. high) ) THEN
			IF ( x_now(3) .LE. high ) THEN
				...
				IF ( CLICK_1( MC, 5 ) .LT. Deepest ) THEN
					Deepest = CLICK_1( MC, 5 )
				END IF
...
!@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
  ...
  END IF
	...
	IF (myid .EQ. 0) ALLOCATE( ROOT_ESC_Deepest_DIST(N_Part) )

	CALL MPI_BARRIER( MPI_COMM_WORLD, ierr )
	CALL MPI_REDUCE( Esc_Deepest_Dist, ROOT_ESC_Deepest_DIST, N_Part, &
	& MPI_DOUBLE_PRECISION, MPI_SUM, 0, MPI_COMM_WORLD, ierr )
	CALL MPI_BARRIER( MPI_COMM_WORLD, ierr )
	IF (myid .EQ. 0) THEN
		OPEN(UNIT=76,FILE='../Data/Escape_Deepest_Distribution.dat',ACCESS='APPEND')
		DO i=1,N_Part
			IF (ROOT_ESC_Deepest_DIST(i) .NE. 0.0D0) WRITE(76,*) ROOT_ESC_Deepest_DIST(i)	
		END DO
		CLOSE(76)
	END IF
