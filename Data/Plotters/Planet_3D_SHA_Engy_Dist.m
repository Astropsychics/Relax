...
f = fopen('../planet_SHA_3D_Energy_Dist.dat'); 
...
imagesc(Engy,Height/1.0D3,LZ)
colormap(hot)
h = colorbar; 
ylabel(h,'Log_{10}   cm^{-2} sec^{-1}','FontSize',16);
set(gca,'YDir','normal'); 
set(gca,'FontSize',16); 
xlabel('SHA Energy [eV/amu]'); 
ylabel('Altitude [km]'); 
print -depsc2 ./Plots/Planet_3D_SHA_Energy_Dist.eps
end
...
if (SLICES == 1)
	...
	tit = sprintf('%d km', D0); 
	title(tit) 
	print -depsc2 ./Plots/Planet_SHA_Energy_Dist_80km.eps
	...
	tit = sprintf('%d km', D1); 
	title(tit) 
	print -depsc2 ./Plots/Planet_SHA_Energy_Dist_85km.eps
	...
	tit = sprintf('%d km', D2); 
	title(tit) 
	print -depsc2 ./Plots/Planet_SHA_Energy_Dist_90km.eps
	...
	tit = sprintf('%d km', D3); 
	title(tit) 
	print -depsc2 ./Plots/Planet_SHA_Energy_Dist_100km.eps
	...
	tit = sprintf('%d km', D4); 
	title(tit) 
	print -depsc2 ./Plots/Planet_SHA_Energy_Dist_125km.eps
	...
	tit = sprintf('%d km', D5); 
	title(tit) 
	print -depsc2 ./Plots/Planet_SHA_Energy_Dist_150km.eps
...
%	axis([0 40 1e-4 1e3])
	legend('80 km','85 km','90 km','100 km','125 km','150 km','Location','Best')
	print -depsc2 ./Plots/Planet_SHA_Energy_Dist_All.eps

	PRINT_DATA = 1; 
	if (PRINT_DATA == 1)
		fo0 = fopen('./Plots/SHA_80km.dat','w'); 
		fo1 = fopen('./Plots/SHA_85km.dat','w'); 
		fo2 = fopen('./Plots/SHA_90km.dat','w'); 
		fo3 = fopen('./Plots/SHA_100km.dat','w'); 
		fo4 = fopen('./Plots/SHA_125km.dat','w'); 
		fo5 = fopen('./Plots/SHA_150km.dat','w'); 
		for i=1:length(Engy)
			fprintf(fo0,'%e %e\n', Engy(i), RZ(z0,i)); 
			fprintf(fo1,'%e %e\n', Engy(i), RZ(z1,i)); 
			fprintf(fo2,'%e %e\n', Engy(i), RZ(z2,i)); 
